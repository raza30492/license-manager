...
export function searchUser (email) {
  ...
  return function (dispatch) {
    ...
    axios.get(window.serviceHost + '/users/search/byEmail?email=' + email, {headers: getHeaders()})
    .then((response) => {
      console.log(response);
      if (response.status == 200) {
...
export function syncUser (after) {
  ...
  return function (dispatch) {
    dispatch({type: c.USER_BUSY});
    axios.get(window.serviceHost + '/users?after=' + after, {headers: getHeaders()})
    .then((response) => {
      console.log(response);
      if (response.status == 200) {
        dispatch({type: c.USER_SYNC_SUCCESS, payload: {resp: response.data.content}});
      }
    }).catch( (err) => {
      console.log(err);
